    ...
    layersOrShapes.forEach((el, layerIndex) => {
        ...
        if (el.ty == 1) {
            const meta = {
                name: el.nm,
                type: el.ty,
                path: `${startingPath}.${layerIndex}`,
                rgba: hexToRGBA(el.sc, 1),
                isSolid: true,
            };
...
export const validNumber = (value) => Math.round(value % 2 === 0 ? value : value - 1);
...
export const removePrefixFromCategoryName = (name) => {
